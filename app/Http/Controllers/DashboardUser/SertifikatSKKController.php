...
class SertifikatSKKController extends Controller
{
    protected $fpdf;
    ...
    {
       //dd($request->all());
        $data_update = PengajuanSKK::where('nama_lengkap', $request->input('pengaju'))
        ->where('tingkat',$request->input('id_tingkat'))
        ->where('kategori',$request->input('id_kategori'))
        ->first();
        $data_update->status = 'Lulus'; 
        $data_update->save();

        $user = DataSKK::where('id_user', $request->input('pengaju'))
        ->where('kategori',$request->input('id_kategori'))
        ->first();
        ...
        $data = SertifikatSKK::where('pengaju', $request->input('pengaju'))->where('kategori',$request->input('kategori'))->firstOrNew();
        ...
        $pdfContent = $this->generatepdf($nama, $tempat, $tanggal, $tingkat, $kategori);
        $pdfFileName = 'sertifikat_skk_' . $data->nama_pengaju . $data->tingkat . $data->kategori . $data->pengaju . '.pdf';
    ...
    public function generatepdf($nama, $tempat, $tanggal, $tingkat, $kategori){
        ob_start();
        ...
        $this->fpdf->SetX(30); 
        $this->fpdf->Cell(0, 8, 'Kategori', 0, 1, 'L');
