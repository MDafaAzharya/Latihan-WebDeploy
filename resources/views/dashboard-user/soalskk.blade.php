...
@section('content')
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Tes SKK </h4>
    ...
    <div class="card">
        <div class="ms-3 mt-3">
            ...
            @endforeach
            @foreach ($nama_kategori as $list)
            <p><strong>Kategori : {{$list->nama_kategori }}</strong></p>
            @endforeach
        </div>
        <h5 class="card-header">Daftar Soal</h5>
        @foreach ($soalSKK as $item)
        <form action="{{ route('skk.soal-skk-user-jawaban') }}" method="post">
        @csrf
            ...
            <div class="row mx-5">
                ...
                @php
                    $jawaban = \App\Models\JawabanSKK::where('soal', $item->id)
    ...
    <div class="modal fade" id="buatSertifikatModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                action="{{ route('generate-sertifikat-skk') }}" enctype="multipart/form-data">
                @csrf
                ...
                @foreach ($nama_pengaju as $item)
                    ...
                    @enderror
                    <input type="hidden" name="id_tingkat" value="{{$list->id}}">
                    </div>
                    @endforeach
                    @foreach ($nama_kategori as $list)
                    <div class="mb-3"> <label for="email" class="form-label">{{ __('Kategori') }}</label>
                        <input type="text" class="form-control @error('kategori') is-invalid @enderror"
                        id="kategori" name="kategori" placeholder="kategori" required autocomplete="kategori" autofocus readonly
                        value="{{ $list->nama_kategori }}" /> @error('kategori')
                        <span class="invalid-feedback" role="alert"> <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                    </div>
                    <input type="hidden" name="id_kategori" value="{{$list->id}}">
                    @endforeach
