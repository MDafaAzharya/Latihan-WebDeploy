            ...
            <div class="card-body">
              <div class="w-full p-2">
                <img
                    ...
                    src="{{ asset('storage/images/' . $prestasi->image) }}"
                    alt="campaign image" />
              </div>
              <h4 class="mb-2">{{ $prestasi->title }}</h4>
            <p>
              {!!$prestasi->text!!}
            </p>
            </div>
          ...
          <div class="card">
            <div class="card-header d-flex justify-content-between">
              <h5 class="card-title m-0 me-2 pt-1 mb-2 d-flex align-items-center">
                <i class="ti ti-list-details ms-n1 me-2"></i> Prestasi Terbaru
              </h5>
            </div>
            <div class="card-body pb-4 mt-2">
            @foreach ($reward->take(2) as $list)
              <div class="card">
                <div class="card-header">
                  <img
                  class="img-fluid w-100"
                  src="{{ asset('storage/images/' . $list->image) }}"
                  alt="campaign image" />
                </div>
                <div class="card-body">
                  <a href="{{ route('prestasi-detail', ['id' => $list->id]) }}">{!! substr($list->text, 0, 80) !!}</a>
                </div>
              </div>
              @endforeach
          </div>
