...
@section('content')
<h4 class="py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Profile </h4>
    ...
    <section>
        <div class="card">
            ...
            action="{{ route('profile.data-profile-update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @foreach($profile as $key => $data)
                <input type="hidden" name="id" value="{{$data->id}}">
                <div class="mb-3"> <label for="email" class="form-label">{{ __('Judul') }}</label>
                    <input type="text" class="form-control @error('judul') is-invalid @enderror"
                        id="judul" name="judul" placeholder="judul" value="{{$data->title}}" required autocomplete="judul" autofocus
                ...
                <div class="mb-3"> <label for="email" class="form-label">{{ __('Pilih Foto') }}</label>
                    <input type="file" class="form-control @error('gambar') is-invalid @enderror"
                        id="gambar" name="gambar" placeholder="gambar" value="{{$data->title}}" required autocomplete="gambar" autofocus
                ...
                <div class="d-flex justify-content-start mb-3">
                    <img src="{{ asset('storage/images/' . $data->image) }}" id="gambar_edit" alt="" srcset="" width="100" height="100">
