...
@section('content')
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Soal SKU </h4>
    ...
    <div class="card">
        <h5 class="card-header">Soal SKU </h5>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="w-px-400 mx-auto">
                        <form id="formAuthentication" class="mb-3" action="{{ route('sku.data-sku-register') }}"
                            method="POST">
                            @csrf
                ...
                <div class="modal-body">
                    <div class="w-px-400 mx-auto">
                        <form id="formAuthentication" class="mb-3" method="post"
                            action="{{ route('sku.data-sku-update') }}">
                            @csrf
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="tingkat" id="tingkat_edit" class="form-select form-control">
                                @foreach ($tingkat as $item)
                                    <option value="{{ $item->id }}">{{ $item->nama_tingkat }}</option>
                                @endforeach
                                </select>
...
@section('page-js')
    <script type="text/javascript">
        ...
        var table = $('#table').DataTable({
            ajax: `{{ route('sku.data-sku-datatable') }}`,
            scrollX: true,
            // searching: true,
            paging: true,
            processing: true,
        ...
        $(document).delegate('.btn-delete', 'click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('dashboard/sku_admin/data-sku-delete') }}/" + id,
                        type: 'get',
                        data: {
                            _token: `{{ csrf_token() }}`,
                            id
        ...
        $(document).delegate('.btn-edit', 'click', function() {
            const id = $(this).data('id');
            $.ajax({
                url: "{{ url('dashboard/sku_admin/data-sku-edit') }}/" + id,
                type: 'get',
                data: {
                    _token: `{{ csrf_token() }}`,
                    id
