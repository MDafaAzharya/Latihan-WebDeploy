...
@section('content')
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">Dashboard /</span> Kategori SKK </h4>
    ...
    <div class="card">
        <h5 class="card-header">Kategori SKK</h5>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="w-px-400 mx-auto">
                        <form id="formAuthentication" class="mb-3" action="{{ route('kategori_skk.data-kategori_skk-register') }}"
                            method="POST">
                            @csrf
                ...
                <div class="modal-body">
                    <div class="w-px-400 mx-auto">
                        <form id="formAuthentication" class="mb-3" method="post"
                            action="{{ route('kategori_skk.data-kategori_skk-update') }}">
                            @csrf
                            <input type="hidden" name="id" id="id">
                            <div class="mb-3"> <label for="email" class="form-label">{{ __('Nama Kategori') }}</label>
                                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                    id="nama_edit" name="nama" placeholder="nama" required autocomplete="kategori" autofocus
                                    value="{{ old('nama') }}" /> @error('nama')
                                    <span class="invalid-feedback" role="alert"> <strong>{{ $message }}</strong>
                                    </span>
                                @enderror
...
@section('page-js')
    <script type="text/javascript">
        ...
        var table = $('#table').DataTable({
            ajax: `{{ route('kategori_skk.data-kategori_skk-datatable') }}`,
            scrollX: true,
            // searching: true,
            paging: true,
            processing: true,
        ...
        $(document).delegate('.btn-delete', 'click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('dashboard/kategori_skk_admin/data-kategori_skk-delete') }}/" + id,
                        type: 'get',
                        data: {
                            _token: `{{ csrf_token() }}`,
                            id
        ...
        $(document).delegate('.btn-edit', 'click', function() {
            ...
            const id = $(this).data('id');
            $.ajax({
                url: "{{ url('dashboard/kategori_skk_admin/data-kategori_skk-edit') }}/" + id,
                type: 'get',
                data: {
                    _token: `{{ csrf_token() }}`,
                    id
                ...
                success: function(resp) {
                    $('#id').val(resp.id);
                    $('#nama_edit').val(resp.nama_kategori);
